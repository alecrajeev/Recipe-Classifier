...
def recipe():
    ...
    ingredients_count = build_ingredients_count(
        recipe_list, unique_cuisines, unique_ingredients, hash_table)
...
def get_probability_of_ingredient(
        cuisine_index,
        ingredient_index,
        ingredients_count,
        unique_cuisines):
...
def get_probability_of_cuisine(
        cuisine_index,
        unique_cuisines,
        number_of_tuples):
...
def print_classified_cuisines(test_recipe_list):
    ...
    for i in xrange(0, np.size(test_recipe_list)):
        print str(test_recipe_list[i].id) + ","
        + test_recipe_list[i].cuisine
    ...
    for i in xrange(0, np.size(test_recipe_list)):
        ...
        for cuisine_index in xrange(0, np.size(unique_cuisines[0])):
            ...
            for k in xrange(0, np.size(test_recipe.ingredients)):
                ...
                # if ingredient_index == -1 then the ingredient wasn't in the
                # training data, so ignore
                if ingredient_index > -1:
                    prod *= get_probability_of_ingredient(
                        cuisine_index,
                        ingredient_index,
                        ingredients_count,
                        unique_cuisines)

            test_recipe.cuisine_probabilities[
                cuisine_index] = prod * get_probability_of_cuisine(
                    cuisine_index, unique_cuisines, number_of_tuples)
...
def build_test_recipe_list(test_data):
    ...
    for i in xrange(0, np.size(test_data)):
        test = test_data[i]
        recipe = TestRecipe(
            test_data[i]["id"], np.array(
                test_data[i]["ingredients"]))
        test_recipe_list.append(recipe)

    return test_recipe_list


def build_ingredients_count(
        recipe_list,
        unique_cuisines,
        unique_ingredients,
        hash_table):
    ingredients_count = np.zeros(
        (np.size(
            unique_cuisines[0]),
            np.size(unique_ingredients)),
        dtype=np.int)
